...
public class CytomineRGBStackMerge extends RGBStackMerge {
    ...
    public static ImagePlus merge(ImagePlus[] images, Color[] imageColors, boolean keepSourceImages) throws Exception {
        CytomineRGBStackMerge rgbsm = new CytomineRGBStackMerge();
        return rgbsm.mergeHyperstacks(images, imageColors, keepSourceImages);
    }

    public ImagePlus mergeHyperstacks(ImagePlus[] images, Color[] imageColors,boolean keep) throws Exception {

        if(images.length!=imageColors.length)
            throw new Exception("Image number != Color number: 1 image = 1 color!");
        ...
        Color[] defaultColors = imageColors;
